...
function RegisterTeacherForm({ onTeacherRegistered }) {
  ...
  const handleSubmit = (event) => {
    ...
    const surname = form['teacher-surname'].value.trim()
    const email = form['teacher-email'].value.trim()
    const password = form['teacher-password'].value
    ...
    <div className='flex items-center justify-center w-full max-w-md p-2 bg-white rounded-lg shadow-lg my-4'>
      ...
      <div className='w-full flex flex-col justify-center items-center mb-1' >
        ...
        <Form onSubmit={handleSubmit} className='w-full flex justify-center'>
          ...
          <Input type='text' id='teacher-name' placeholder='Name' /><br />
          {error?.isNameError && <span className='text-[#C13E65]'>{error.message}</span>}

          <Input type='text' id='teacher-surname' placeholder='Surname' /><br />
          {error?.isSurnameError && <span className='text-[#C13E65]'>{error.message}</span>}

          <Input type="text" id="teacher-email" placeholder='Email' /><br />
          {error?.isEmailError && <span className='text-[#C13E65]'>{error.message}</span>}

          <Input type='password' id='teacher-password' placeholder='Password' /><br />
